...
async def help_handler(message: types.Message):
    await message.answer("Просто отправьте мне вопрос, связанный с компьютерами, и я постараюсь помочь. Доступные команды: /start — начать /help — помощь /about — о боте")

@dp.message_handler(commands=["about"])
async def about_handler(message: types.Message):
    await message.answer("🧠 Я использую ChatGPT для генерации ответов на вопросы по ремонту и настройке компьютеров. Разработано NeoHelp.")
